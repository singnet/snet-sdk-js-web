...
class RegistryContract {
    ...
    _toPaddedHex(string, bit = 64) {
        ...
        const paddedHex = hex.padEnd(
            bit * (Math.floor(hexLength / bit) + 1) + additionalBit,
            '0'
        );
        return paddedHex;
    }
    ...
    createOrganization(orgId, orgMetadataURI, members) {
        ...
        return this._contract.methods.createOrganization(
            enhancedOrgId,
            enhancedOrgMetadataURI,
            [...members]
        );
    }
    ...
    changeOrganizationMetadataURI(orgId, orgMetadataURI) {
        const enhancedOrgId = this._toPaddedHex(orgId);
        const enhancedOrgMetadataURI = this._toPaddedHex(orgMetadataURI);
        return this._contract.methods.changeOrganizationMetadataURI(
            enhancedOrgId,
            enhancedOrgMetadataURI
        );
    }
    ...
    addOrganizationMembers(orgId, newMembers) {
        const enhancedOrgId = this._toPaddedHex(orgId);
        return this._contract.methods.addOrganizationMembers(
            enhancedOrgId,
            newMembers
        );
    }
    ...
    removeOrganizationMembers(orgId, existingMembers) {
        const enhancedOrgId = this._toPaddedHex(orgId);
        return this._contract.methods.removeOrganizationMembers(
            enhancedOrgId,
            existingMembers
        );
    }
    ...
    createServiceRegistration(orgId, serviceId, serviceMetadataURI) {
        ...
        const enhancedServiceMetadataURI =
            this._toPaddedHex(serviceMetadataURI);
    ...
    getServiceRegistrationById(orgId, serviceId) {
        const enhancedOrgId = this._toPaddedHex(orgId);
        const enhancedServiceId = this._toPaddedHex(serviceId);
        return this._contract.methods.getServiceRegistrationById(
            enhancedOrgId,
            enhancedServiceId
        );
    }
